...
python3 testcases/toss_contract_smoke.py   # read-only venue contract check (needs TOSS_* creds; exit 0/1/2)
python3 testcases/rr_ab_study.py           # R/R basis A/B study behind the threshold calibration
...
- **`auto/watchdog.py`** — Liveness signalling: the auto loop publishes `runtime/heartbeats/{auto,monitor}.json` each sweep *and* every 30s while sleeping (`sleep_with_heartbeat`), so a 300s interval never looks stale. Written atomically and read by `updated_epoch` inside the file, never by mtime — backup tools and editors touch mtime. `check_heartbeat` returns a **structured `state`** (`ok`/`absent`/`stopped`/`failed`/`stale`/`invalid`/`skew`), not just `healthy`: a bot the operator switched off and a bot that died are both unhealthy, but only the second is an alarm. `ALARMING_STATES` is the shared definition — the safety bar renders `stopped` as an `info` line and `absent` silently, while `runner watchdog` (out-of-process, Telegram) pages on alarming states and on `absent` after its startup grace. Deciding this from the reason *string* is what previously made every clean Ctrl+C warn. The heartbeat only reports; what actually covers a position while the bot is down is the venue-side protective stop.
- **`auto/sizing.py`** — `compute_position_size()`: shares = total_value × `risk_per_trade_pct` ÷ per-share-risk, capped by available cash **and** by `max_position_pct` of account value (a 2.5% stop with 1% risk would otherwise size to ~40% of equity). The fixed-quantity path enforces the same cap in the orchestrator pre-flight. `usable_cash()` (KIS paper cash=0 fallback) lives here and is shared with the orchestrator pre-flight. **`sizing_base_value()` decides what "% of account" means**: the FX-unified portfolio (`portfolio_value` capability, Toss only — sleeves summed at the 매매기준율/midRate, never the spread-bearing buy rate, 60s TTL) when the broker can price it, else the trade's own currency sleeve (legacy behaviour). Risk budget and `max_position_pct` both use this base — the fixed-quantity pre-flight in the orchestrator shares the same helper so the two caps can't disagree — but the **cash affordability cap stays per-sleeve** (a KRW deposit cannot settle a USD buy). Valuation failure falls back to the sleeve, which is the conservative direction: the sleeve is never larger than the portfolio, so failures shrink positions instead of inflating them. The daily-loss breaker uses the **same base**: positions are sized as a % of it, so losses are measured against it too — a sleeve denominator would trip the breaker on one normally-sized stop-out whenever most of the account sits in the other currency.
...
- **`data/fundamentals_sources.py`** — **Multi-source fundamentals chain.** A missing fetch is not neutral: the analyzer scores 0/10 ("평가 불가"), capping a 30-point total at 20 and putting `min_score` out of reach — indistinguishable from a genuinely weak company. `resolve_fundamentals()` tries sources in order, takes the first that answers, and **records which one did** (a fallback nobody can attribute is one nobody notices is broken). US: yfinance → SEC EDGAR → fixtures. KR: Naver Finance → KIS (only when a client is supplied) → fixtures. SEC EDGAR uses **NetIncomeLoss, not EPS** — EDGAR serves as-reported figures, so NVDA's pre-10:1-split EPS would make a growing quarter read −87%; net income is split-immune. YoY pairs **by date within ±45 days**, never by list position, because sources skip quarters (EDGAR has no standalone Q4). `_yoy` refuses non-positive bases and >20× jumps — Ford's +3200% off a rounding-error quarter would otherwise collect the scorer's top tier. Resolution prefers **depth, not merely first-answer**: yfinance publishes only 5 quarterly columns → exactly one YoY pair, which left the scorer's Q-over-Q acceleration bonus (the "C" in CANSLIM) permanently dead for US names, so the chain keeps looking until a source clears 3 quarters and returns the deepest. `eps_surprise` is grafted from the shallower yfinance result onto the deeper one when the newest quarter labels agree — otherwise choosing depth would silently trade +1 for +2. Earnings and revenue XBRL concepts are both **merged across tags**, since filers migrate them mid-history (Broadcom's `NetIncomeLoss` stops in 2019 and continues under `ProfitLoss`). 24h disk cache at `data/cache/fundamentals/`; empty results are never cached.
...
- **`web/`** — `http.server` dashboard on port 8501. `server.py` routes GET/POST to handler modules split by concern: `handlers_analysis`, `handlers_orders`, `handlers_portfolio`, `handlers_config`, `handlers_mock`, `handlers_safety`. The UI is organised by **what the operator is asking**, not by feature: an always-on safety bar (`/api/safety` — kill switch, breakers, order halts, heartbeats, armed stops, unified assets) sits above every screen; the ops page (landing) pairs positions and the decision feed with a gate panel (`/api/gates`, mirroring `run_auto_iteration`'s guard order) answering "why isn't it buying"; the account page leads with reconciliation (`/api/reconcile` — bot belief vs venue truth, holdings classified bot/protected/manual, real fractional quantities); settings are grouped by blast radius (전략 → 리스크 → 연결) and write `config.yaml` through `handle_save_config`, which bounds-checks every field server-side and rejects the whole update on any error. Shared rule across all of it: **a value that could not be read renders as a warning, never as green**. `autopilot_state.py` holds the background auto-pilot thread state. Static UI in `static/dashboard.html`, styled by the **committed** `static/dashboard.css` — Tailwind, the Phosphor glyphs actually used (subset, base64-inlined) and nothing else. The page loads zero external hosts: an ops screen must render when the network is the thing that broke. Regenerate with `python3 tools/build_dashboard_assets.py` after touching any class; `tests/test_dashboard_assets.py` fails when the CSS has drifted from the markup, so a forgotten rebuild is a red test rather than a silently unstyled element. Assets are served by exact-match routes (`/static/dashboard.css`), never by joining a requested path.
- **`gui.py`** — Thin Tkinter desktop wrapper (`bot-gui`).

**Reporting & observability**
- **`notify.py`** — Best-effort Telegram alerts (buy submitted, exits, forced exits, circuit breaker, kill switch). Configured via `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in `.env` (editable from the settings page, token masked on read-back); silent no-op when unset — so the safety bar warns when the auto heartbeat is healthy and the channel is not configured, the one combination where nothing can reach the operator. A stopped bot is not flagged: a permanently amber bar for someone who never wanted Telegram is the same false alarm as any other. De-duplicates identical alerts for 30 min so the auto-pilot loop can't spam. stdlib urllib only.
...
State files at repo root: `pending_orders.json` (queue — long-settled groups auto-archive to `logs/orders_archive/YYYY-MM.json` after 7 days, keeping the hot file small since every `update()` rewrites it), `mock_orders.json` (mock broker), `news_cache.json` (LLM cache), `.kis_token_*.json` (KIS token), `logs/*.jsonl` (audit).

### Configuration

- `config.yaml` / `config.example.yaml` — `broker`, `min_score`, `min_rr`, `risk_per_trade_pct`, `max_positions`, `default_market`, `data_dir`, `approval_queue`, plus risk guards: `max_position_pct` (default 20, 0=off), `daily_loss_limit_pct` (default 3, 0=off), `stale_order_minutes` (default 60, 0=off), `protected_tickers` (comma-separated; the bot must never buy, sell, or arm a stop on these — enforced at `ApprovalQueue.enqueue()` so **no** code path can trade them, with independent guards in the auto sweep and position manager), the entry-style toggle `require_breakout` (default false), and `protective_stop` (default false — enabling it places real standing orders at the venue).
...
- `.env` — **Toss creds** (`TOSS_CLIENT_ID`, `TOSS_CLIENT_SECRET`, optional `TOSS_ACCOUNT_SEQ`; `TOSS_ENABLE_LIVE_ORDERS=false` until the read-only smoke test and a tiny manual order pass; `TOSS_ALLOW_HIGH_VALUE_ORDERS=false` keeps the ₩100M fat-finger refusal local), legacy KIS creds (`KIS_APP_KEY`, `KIS_APP_SECRET`, `KIS_ACCOUNT_NO`, `KIS_MODE=paper`), optional `OPENAI_API_KEY` / `OPENAI_MODEL`, optional `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` for alerts. Also honors `NEWS_CACHE_*`, `BOT_BROKER`, `BOT_APPROVAL_QUEUE`, `BOT_KILL_SWITCH`, `BOT_HEARTBEAT_DIR`.
